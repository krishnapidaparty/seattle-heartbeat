...
export function createAguiHttpHandler(api: OpenClawPluginApi) {
  ...
  return async function handleAguiRequest(
    ...
    // Build body from messages + context
    ...
    // Append AG-UI context (from useCopilotReadable) so the agent sees app state
    const contextItems = Array.isArray((input as { context?: unknown[] }).context)
      ? (input as { context?: { description?: string; value?: string }[] }).context!
      : [];
    const contextBlock = contextItems
      .filter((c) => c.description || c.value)
      .map((c) => `[${c.description ?? "context"}]: ${c.value ?? ""}`)
      .join("\n");
    const fullBody = contextBlock
      ? `${messageBody}\n\n--- App context ---\n${contextBlock}`
      : messageBody;
