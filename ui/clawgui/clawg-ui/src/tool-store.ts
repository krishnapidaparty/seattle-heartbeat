...
export function markClientToolNames(
  ...
  clientToolNames.set(sessionKey, new Set(names.map(n => n.toLowerCase())));
}
...
export function isClientTool(
  ...
  return clientToolNames.get(sessionKey)?.has(toolName.toLowerCase()) ?? false;
}
...
// Tracks whether any tool call was emitted in the current run. When a text
// message is about to be emitted and this flag is set, the http-handler
// splits into a new run so tool events and text events live in separate
// runs (per AG-UI protocol best practice).
